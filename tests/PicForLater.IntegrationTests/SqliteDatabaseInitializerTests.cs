...
public sealed class SqliteDatabaseInitializerTests
{
    ...
    public async Task Initialize_CreatesCurrentSchemaWithoutBackupForANewDatabase()
    {
        ...
        Assert.Equal(16L, await ExecuteScalarLongAsync(connection, "PRAGMA user_version;"));
    ...
    public async Task Initialize_IsIdempotentWhenSchemaIsCurrent()
    {
        ...
        Assert.Equal(16, secondResult.PreviousVersion);
        Assert.Equal(16, secondResult.CurrentVersion);
    ...
    public async Task Migration12_PreservesPublishedV11AndExpandsStructuredOutputModeIncrementally()
    {
        ...
        var upgraded = await new SqliteDatabaseInitializer(
            temporaryRoot.Paths,
            SqliteSchema.Migrations.Take(12).ToArray())
            .InitializeAsync();
    ...
    [Fact]
    public async Task Migration13_NormalizesOutboxIdsAndAddsReminderQueryIndexes()
    {
        using var temporaryRoot = new TemporaryAppDataRoot();
        var v12Initializer = new SqliteDatabaseInitializer(
            temporaryRoot.Paths,
            SqliteSchema.Migrations.Take(12).ToArray());
        await v12Initializer.InitializeAsync();
        ...
        await ExecuteNonQueryAsync(
            ...
            VALUES (
                '{outboxId:N}', NULL, 'migration13', 2, NULL,
                NULL, NULL, NULL, 1, 0, '2026-08-27T00:00:00.0000000+00:00',
                NULL, '2026-08-27T00:00:00.0000000+00:00',
                '2026-08-27T00:00:00.0000000+00:00', NULL);
            """);

        var upgraded = await new SqliteDatabaseInitializer(
                temporaryRoot.Paths,
                SqliteSchema.Migrations.Take(13).ToArray())
            .InitializeAsync();

        Assert.Equal(12, upgraded.PreviousVersion);
        ...
        Assert.Equal(
            ...
            await ExecuteScalarStringAsync(
                connection,
                "SELECT Id FROM ReminderNotificationOutbox WHERE SchedulerId = 'migration13';"));

        await using var reminderPlan = connection.CreateCommand();
        reminderPlan.CommandText =
            "EXPLAIN QUERY PLAN SELECT MIN(DueAtUtc) FROM Reminders WHERE State = 1;";
        var reminderPlanDetails = new List<string>();
        await using (var reader = await reminderPlan.ExecuteReaderAsync())
        {
            while (await reader.ReadAsync())
            {
                reminderPlanDetails.Add(reader.GetString(3));
            }
        }

        Assert.Contains(
            reminderPlanDetails,
            detail => detail.Contains("IX_Reminders_State_DueAtUtc_Id", StringComparison.Ordinal));

        await using var outboxPlan = connection.CreateCommand();
        outboxPlan.CommandText =
            $"EXPLAIN QUERY PLAN UPDATE ReminderNotificationOutbox SET State = 2 WHERE Id = '{outboxId:D}';";
        var outboxPlanDetails = new List<string>();
        await using (var reader = await outboxPlan.ExecuteReaderAsync())
        {
            while (await reader.ReadAsync())
            {
                outboxPlanDetails.Add(reader.GetString(3));
            }
        }
        ...
        Assert.Contains(
            ...
            detail => detail.Contains(
                "sqlite_autoindex_ReminderNotificationOutbox_1",
                StringComparison.Ordinal));
    }

    [Fact]
    public async Task Migration14_PreservesExistingRowsAndAllowsLocalSendSourceKind()
    {
        using var temporaryRoot = new TemporaryAppDataRoot();
        var v13Initializer = new SqliteDatabaseInitializer(
            temporaryRoot.Paths,
            SqliteSchema.Migrations.Take(13).ToArray());
        await v13Initializer.InitializeAsync();
        var assetId = Guid.NewGuid().ToString("D");
        var existingItemId = Guid.NewGuid().ToString("D");
        var existingImportId = Guid.NewGuid().ToString("D");
        var existingAnalysisId = Guid.NewGuid().ToString("D");
        await ExecuteNonQueryAsync(
            temporaryRoot.Paths.DatabasePath,
            $"""
            INSERT INTO ImageAssets (
            ...
            VALUES (
                '{assetId}', '{new string('a', 64)}', 'assets/originals/existing.png', NULL,
                'image/png', 1, 1, 1, '2026-08-28T00:00:00.0000000+00:00');
            ...
            INSERT INTO ImageItems (
                ...
                CreatedAtUtc, UpdatedAtUtc, DeletedAtUtc)
            VALUES (
                '{existingItemId}', '{assetId}', 'existing.png', 1, 'Existing', '',
                1, 1, 1, 0, '2026-08-28T00:00:00.0000000+00:00',
            ...
            INSERT INTO ImportJobs (
                ...
                CompletedAtUtc)
            VALUES (
                '{existingImportId}', NULL, 'assets/originals/existing.png', 'existing.png',
                2, 3, '{new string('a', 64)}', '{existingItemId}', 0,
                NULL, NULL, '2026-08-28T00:00:00.0000000+00:00',
            ...
            INSERT INTO AnalysisJobs (
                ...
                CreatedAtUtc, UpdatedAtUtc, CompletedAtUtc)
            VALUES (
                '{existingAnalysisId}', '{existingItemId}', 1, 0, 1, 0,
                '2026-08-28T00:00:00.0000000+00:00', NULL, NULL,
                '2026-08-28T00:00:00.0000000+00:00',
                '2026-08-28T00:00:00.0000000+00:00', NULL);
            """);
        ...
        Assert.Equal(1L, await ExecuteScalarLongAsync(
            connection,
            $"SELECT SourceKind FROM ImageItems WHERE Id = '{existingItemId}';"));
        Assert.Equal(2L, await ExecuteScalarLongAsync(
            connection,
            $"SELECT SourceKind FROM ImportJobs WHERE Id = '{existingImportId}';"));
        Assert.Equal(1L, await ExecuteScalarLongAsync(
            connection,
            $"SELECT COUNT(*) FROM AnalysisJobs WHERE Id = '{existingAnalysisId}';"));

        var localSendAssetId = Guid.NewGuid().ToString("D");
        var localSendItemId = Guid.NewGuid().ToString("D");
        var localSendImportId = Guid.NewGuid().ToString("D");
        await ExecuteNonQueryAsync(
            temporaryRoot.Paths.DatabasePath,
            $"""
            INSERT INTO ImageAssets (
                ...
                MediaType, ByteLength, PixelWidth, PixelHeight, CreatedAtUtc)
            VALUES (
                '{localSendAssetId}', '{new string('b', 64)}', 'assets/originals/localsend.png', NULL,
                'image/png', 1, 1, 1, '2026-08-28T01:00:00.0000000+00:00');
            INSERT INTO ImageItems (
                Id, AssetId, OriginalFileName, SourceKind, Title, Summary,
                TitleSource, SummarySource, AnalysisState, Revision,
                CreatedAtUtc, UpdatedAtUtc, DeletedAtUtc)
            VALUES (
                '{localSendItemId}', '{localSendAssetId}', 'localsend.png', 3, 'LocalSend', '',
                1, 1, 1, 0, '2026-08-28T01:00:00.0000000+00:00',
                '2026-08-28T01:00:00.0000000+00:00', NULL);
            INSERT INTO ImportJobs (
                Id, StagingRelativePath, FinalRelativePath, OriginalFileName,
                SourceKind, State, ContentHash, ImageItemId, AttemptCount,
                LeaseExpiresAtUtc, LastErrorCode, CreatedAtUtc, UpdatedAtUtc,
                CompletedAtUtc)
            VALUES (
                '{localSendImportId}', NULL, 'assets/originals/localsend.png', 'localsend.png',
                3, 3, '{new string('b', 64)}', '{localSendItemId}', 0,
                NULL, NULL, '2026-08-28T01:00:00.0000000+00:00',
                '2026-08-28T01:00:00.0000000+00:00',
                '2026-08-28T01:00:00.0000000+00:00');
            """);
        ...
        await Assert.ThrowsAsync<SqliteException>(() => ExecuteNonQueryAsync(
            ...
            INSERT INTO ImageItems (
                ...
                CreatedAtUtc, UpdatedAtUtc, DeletedAtUtc)
            VALUES (
                '{Guid.NewGuid():D}', '{localSendAssetId}', 'invalid.png', 4, 'Invalid', '',
                1, 1, 1, 0, '2026-08-28T01:00:00.0000000+00:00',
                '2026-08-28T01:00:00.0000000+00:00', NULL);
            """));
    ...
    public async Task Migration15_DefaultsExistingSettingsAndConstrainsOutputLanguage()
    {
        ...
        var upgraded = await new SqliteDatabaseInitializer(
                temporaryRoot.Paths,
                SqliteSchema.Migrations.Take(15).ToArray())
            .InitializeAsync();
    ...
    [Fact]
    public async Task Migration16_AddsNotesWithEmptyDefaultAndPreservesExistingImageRows()
    {
        using var temporaryRoot = new TemporaryAppDataRoot();
        await new SqliteDatabaseInitializer(
                temporaryRoot.Paths,
        ...
        const string now = "2026-09-12T00:00:00.0000000+00:00";
        await ExecuteNonQueryAsync(
            temporaryRoot.Paths.DatabasePath,
            $"""
            INSERT INTO ImageAssets (
                ...
                MediaType, ByteLength, PixelWidth, PixelHeight, CreatedAtUtc)
            VALUES (
                '{assetId:D}', '{new string('c', 64)}', 'assets/originals/legacy-v15.png', NULL,
                'image/png', 1, 1, 1, '{now}');
            INSERT INTO ImageItems (
                Id, AssetId, OriginalFileName, SourceKind, Title, Summary,
                TitleSource, SummarySource, AnalysisState, Revision,
                CreatedAtUtc, UpdatedAtUtc, DeletedAtUtc)
            VALUES (
                '{itemId:D}', '{assetId:D}', 'legacy-v15.png', 1, 'Legacy title', 'Legacy summary',
                1, 2, 4, 7, '{now}', '{now}', NULL);
            """);

        var upgraded = await new SqliteDatabaseInitializer(temporaryRoot.Paths)
            .InitializeAsync();

        Assert.Equal(15, upgraded.PreviousVersion);
        Assert.Equal(16, upgraded.CurrentVersion);
        Assert.NotNull(upgraded.BackupFilePath);
        await using var connection = await OpenAsync(temporaryRoot.Paths.DatabasePath);
        Assert.Equal(
            ...
            await ExecuteScalarStringAsync(
                connection,
                $"SELECT Notes FROM ImageItems WHERE Id = '{itemId:D}';"));
        Assert.Equal(
            "Legacy title",
            await ExecuteScalarStringAsync(
                connection,
                $"SELECT Title FROM ImageItems WHERE Id = '{itemId:D}';"));
        Assert.Equal(
            "Legacy summary",
            await ExecuteScalarStringAsync(
                connection,
                $"SELECT Summary FROM ImageItems WHERE Id = '{itemId:D}';"));

        var secondInitialization = await new SqliteDatabaseInitializer(temporaryRoot.Paths)
            .InitializeAsync();
        Assert.Equal(16, secondInitialization.PreviousVersion);
        Assert.Equal(16, secondInitialization.CurrentVersion);
        Assert.Null(secondInitialization.BackupFilePath);
    }
    ...
    public async Task PendingMigration_CreatesVerifiedBackupBeforeCommit()
    {
        ...
        var migrations = SqliteSchema.Migrations
            .Concat([new SqliteMigration(17, "test-upgrade", "CREATE TABLE UpgradeMarker (Id INTEGER PRIMARY KEY);")])
            .ToArray();
        ...
        Assert.Equal(16, result.PreviousVersion);
        Assert.Equal(17, result.CurrentVersion);
        ...
        Assert.Equal(16L, await ExecuteScalarLongAsync(backup, "PRAGMA user_version;"));
        ...
        Assert.Equal(17L, await ExecuteScalarLongAsync(upgraded, "PRAGMA user_version;"));
    }
    ...
    public async Task FailedMigration_RollsBackAndPreservesMainDatabaseAndBackup()
    {
        ...
        Assert.Equal(16L, await ExecuteScalarLongAsync(main, "SELECT MAX(Version) FROM SchemaMigrations;"));
        Assert.Equal(16L, await ExecuteScalarLongAsync(main, "PRAGMA user_version;"));
        Assert.Equal("ok", await ExecuteScalarStringAsync(main, "PRAGMA quick_check;"));
    }

    [Fact]
    public async Task ForeignKeyDisabledMigration_RollsBackWhenForeignKeyCheckFails()
    {
        using var temporaryRoot = new TemporaryAppDataRoot();
        await new SqliteDatabaseInitializer(temporaryRoot.Paths).InitializeAsync();
        var migrations = SqliteSchema.Migrations
        ...
        await using var connection = await OpenAsync(temporaryRoot.Paths.DatabasePath);
        Assert.Equal(16L, await ExecuteScalarLongAsync(connection, "PRAGMA user_version;"));
        Assert.Equal(16L, await ExecuteScalarLongAsync(
            connection,
            "SELECT COUNT(*) FROM SchemaMigrations;"));
        Assert.Equal(0L, await ExecuteScalarLongAsync(
            connection,
            "SELECT COUNT(*) FROM AnalysisJobs WHERE Id = '00000000-0000-0000-0000-000000000015';"));
    }
    ...
    public async Task ConcurrentPendingMigration_IsAppliedOnce()
    {
        ...
        await new SqliteDatabaseInitializer(temporaryRoot.Paths).InitializeAsync();
        var migrations = SqliteSchema.Migrations
            .Concat([new SqliteMigration(17, "concurrent-test-upgrade", "CREATE TABLE ConcurrentMarker (Id INTEGER PRIMARY KEY);")])
            .ToArray();
        ...
        Assert.All(results, result => Assert.Equal(17, result.CurrentVersion));
        Assert.Single(Directory.EnumerateFiles(temporaryRoot.Paths.BackupDirectoryPath, "*.db"));
        await using var connection = await OpenAsync(temporaryRoot.Paths.DatabasePath);
        Assert.Equal(17L, await ExecuteScalarLongAsync(connection, "SELECT COUNT(*) FROM SchemaMigrations;"));
    ...
    public async Task PendingMigration_RechecksBackupNeedAfterWaitingForMigrationLock()
    {
        ...
        var releaseLockBoundary = new TaskCompletionSource<bool>(
            TaskCreationOptions.RunContinuationsAsynchronously);
        var migrations = SqliteSchema.Migrations
            .Concat([new SqliteMigration(17, "racing-upgrade", "CREATE TABLE RacingMarker (Id INTEGER PRIMARY KEY);")])
            .ToArray();
